...
// ✅ Export the base URL for file downloads
...
export const adminAPI = {
  ...
  // ✅ ADD THIS - Missing function for ZIP upload with photos
  bulkUploadWithZip: async (zipFile: File) => {
    try {
      ...
      const response = await apiClient.post('/admin/students/bulk-upload-zip', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
        timeout: 60000, // Longer timeout for ZIP files
      });
      return response.data;
    } catch (error: any) {
...
export const classAPI = {
  ...
  // ✅ ADD THIS - Missing function (Classes.tsx uses this)
  getAllSections: async () => {
    try {
      const response = await apiClient.get('/admin/sections');
      return response.data;
...
export const assignmentAPI = {
  ...
  // ✅ ADD THIS - Missing function for file URLs
